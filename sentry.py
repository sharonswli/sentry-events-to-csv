...
def _make_get_request(url, entries = []):
  res = requests.get(url, headers = {"Authorization": f"Bearer {os.getenv('SENTRY_API_KEY')}"})
  ...
  if res.status_code == 200:
    ...
    print(f"Cannot fetch issues by project: Received {res.status_code} status code.")
    return None
...
def _check_next_pagination(link):
  ...
  next_result = re.search(r"results=\"(true|false)\"", next_pagination[2]).group(1)
